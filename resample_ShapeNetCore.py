...
def define_options_parser():
    ...
    parser.add_argument(
        "data_path", type=str, help="Path to repacked .h5 dataset file."
    )
    return parser
...
def process(orig_data, resampled_data, data_inds, part):
    ...
    for i, ts in enumerate(data_inds):
        resampled_data["{}_vertices_c_bounds".format(part)][i + 1] = resampled_data[
            "{}_vertices_c_bounds".format(part)
        ][i] + (
        ...
        resampled_data["{}_vertices_c".format(part)][
            resampled_data["{}_vertices_c_bounds".format(part)][i] :
        ] = orig_data["{}_vertices_c".format(ts[0])][
            orig_data["{}_vertices_c_bounds".format(ts[0])][ts[1]] : orig_data[
                "{}_vertices_c_bounds".format(ts[0])
            ][ts[1] + 1]
        ]

        resampled_data["{}_faces_bounds".format(part)][i + 1] = resampled_data[
            "{}_faces_bounds".format(part)
        ][i] + (
        ...
        resampled_data["{}_faces_vc".format(part)][
            resampled_data["{}_faces_bounds".format(part)][i] :
        ] = orig_data["{}_faces_vc".format(ts[0])][
            orig_data["{}_faces_bounds".format(ts[0])][ts[1]] : orig_data[
                "{}_faces_bounds".format(ts[0])
            ][ts[1] + 1]
        ]

        resampled_data["{}_orig_s".format(part)][i] = orig_data[
            "{}_orig_s".format(ts[0])
        ][ts[1]]
        resampled_data["{}_orig_c".format(part)][i] = orig_data[
            "{}_orig_c".format(ts[0])
        ][ts[1]]

        resampled_data["{}_bbox_s".format(part)][i] = orig_data[
            "{}_bbox_s".format(ts[0])
        ][ts[1]]
        resampled_data["{}_bbox_c".format(part)][i] = orig_data[
            "{}_bbox_c".format(ts[0])
        ][ts[1]]

        resampled_data["{}_labels".format(part)][i] = orig_data[
            "{}_labels".format(ts[0])
        ][ts[1]]
...
def main():
    ...
    for c in range(55):
        ...
        data = list(
            zip(
                list(map(lambda j: i2op[j], sample_i)),
                list(map(lambda j: i2oi[j], sample_i)),
            )
        )
    ...
    for key in orig_data.keys():
        if "bounds" in key or "labels" in key or "orig" in key or "bbox" in key:
            resampled_data.create_dataset(
                key, shape=orig_data[key].shape, dtype=orig_data[key].dtype
            )
        else:
            resampled_data.create_dataset(
                key,
                shape=(0,) + orig_data[key].shape[1:],
                maxshape=(None,) + orig_data[key].shape[1:],
                dtype=orig_data[key].dtype,
            )
