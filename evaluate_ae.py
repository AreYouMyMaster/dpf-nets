...
def define_options_parser():
    ...
    parser.add_argument(
        "sampled_cloud_size", type=int, help="Number of sampled points."
    )
    parser.add_argument(
        "mode",
        type=str,
        help="Prediction mode (training / evaluating / generating / predicting).",
    )
    parser.add_argument(
        "--orig_scale_evaluation",
        action="store_true",
...
eval_dataset = ShapeNetCoreDataset(
    config["path2data"],
    part=args.part,
    meshes_fname=config["meshes_fname"],
    cloud_size=config["cloud_size"],
    return_eval_cloud=True,
    return_original_scale=config["cloud_rescale2orig"]
    or config["orig_scale_evaluation"],
...
path2checkpoint = os.path.join(
    config["path2save"], "models", "DPFNets", config["model_name"]
)
