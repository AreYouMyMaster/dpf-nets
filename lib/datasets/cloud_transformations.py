...
class Scale2OrigCloud(object):
    ...
    def __call__(self, sample):
        ...
        if self.do_recenter:
            ...
            if "eval_cloud" in sample:
                sample["eval_cloud"] = sample["eval_cloud"] + sample["orig_c"].reshape(
                    -1, 1
                )
        return sample
...
class TranslateCloud(object):
    def __init__(self, **kwargs):
        self.shift = np.array(
            kwargs["cloud_translate_shift"], dtype=np.float32
        ).reshape(-1, 1)
...
class AddNoise2Cloud(object):
    ...
    def __call__(self, sample):
        sample["cloud"] += np.random.normal(
            scale=self.scale, size=sample["cloud"].shape
        ).astype(np.float32)
        if "eval_cloud" in sample:
            sample["eval_cloud"] += np.random.normal(
                scale=self.scale, size=sample["eval_cloud"].shape
            ).astype(np.float32)
        return sample
