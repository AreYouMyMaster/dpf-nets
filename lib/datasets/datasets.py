...
class ShapeNetCoreDataset(Dataset):
    ...
    def choose_part(self, part):
        self.part = part
        with h5.File(
            os.path.join(self.path2data, self.meshes_fname),
            "r",
            libver="latest",
            swmr=True,
        ) as fin:
            if self.sample_labels:
                self.labels = np.zeros(
                    (fin[part + "_labels"].shape[0], 55), dtype=np.float32
                )
                self.labels[
                    np.arange(self.labels.shape[0]), np.array(fin[part + "_labels"])
                ] = 1.0

            self.vertices_c_bounds = np.empty(
                fin[part + "_vertices_c_bounds"].shape, dtype=np.uint64
            )
            fin[part + "_vertices_c_bounds"].read_direct(self.vertices_c_bounds)

            self.faces_bounds = np.empty(
                fin[part + "_faces_bounds"].shape, dtype=np.uint64
            )
            fin[part + "_faces_bounds"].read_direct(self.faces_bounds)

            if self.return_original_scale:
                self.original_centers = np.empty(
                    fin[part + "_orig_c"].shape, dtype=np.float32
                )
                fin[part + "_orig_c"].read_direct(self.original_centers)
                self.original_scales = np.empty(
                    fin[part + "_orig_s"].shape, dtype=np.float32
                )
                fin[part + "_orig_s"].read_direct(self.original_scales)

            if self.return_bbox_scale:
                self.bbox_centers = np.empty(
                    fin[part + "_bbox_c"].shape, dtype=np.float32
                )
                fin[part + "_bbox_c"].read_direct(self.bbox_centers)
                self.bbox_scales = np.empty(
                    fin[part + "_bbox_s"].shape, dtype=np.float32
                )
                fin[part + "_bbox_s"].read_direct(self.bbox_scales)

            if self.chosen_label is not None:
                self.chosen_label_inds = (
                    np.array(fin[part + "_labels"], dtype=np.uint8) == self.chosen_label
                ).nonzero()[0]
    ...
    def __getitem__(self, i):
        ...
        vertices_c = np.array(
            self.data_file[self.part + "_vertices_c"][
                self.vertices_c_bounds[i] : self.vertices_c_bounds[i + 1]
            ],
            dtype=np.float32,
        )
        faces_vc = np.array(
            self.data_file[self.part + "_faces_vc"][
                self.faces_bounds[i] : self.faces_bounds[i + 1]
            ],
            dtype=np.uint32,
        )
        sample = sample_cloud(
...
class ShapeNetAllDataset(Dataset):
    ...
    def choose_part(self, part):
        ...
        with h5.File(
            ...
            if self.sample_labels:
                self.labels = np.zeros(
                    (fsh[part + "_labels"].shape[0], 55), dtype=np.float32
                )
                self.labels[
                    np.arange(self.labels.shape[0]), np.array(fsh[part + "_labels"])
                ] = 1.0

            self.vertices_c_bounds = np.empty(
                fsh[part + "_vertices_c_bounds"].shape, dtype=np.uint64
            )
            fsh[part + "_vertices_c_bounds"].read_direct(self.vertices_c_bounds)

            self.faces_bounds = np.empty(
                fsh[part + "_faces_bounds"].shape, dtype=np.uint64
            )
            fsh[part + "_faces_bounds"].read_direct(self.faces_bounds)

            if self.return_original_scale:
                self.original_centers = np.empty(
                    fsh[part + "_orig_c"].shape, dtype=np.float32
                )
                fsh[part + "_orig_c"].read_direct(self.original_centers)
                self.original_scales = np.empty(
                    fsh[part + "_orig_s"].shape, dtype=np.float32
                )
                fsh[part + "_orig_s"].read_direct(self.original_scales)

            if self.return_bbox_scale:
                self.bbox_centers = np.empty(
                    fsh[part + "_bbox_c"].shape, dtype=np.float32
                )
                fsh[part + "_bbox_c"].read_direct(self.bbox_centers)
                self.bbox_scales = np.empty(
                    fsh[part + "_bbox_s"].shape, dtype=np.float32
                )
                fsh[part + "_bbox_s"].read_direct(self.bbox_scales)

            if self.chosen_label is not None:
                self.chosen_label_inds = (
                    np.array(fsh[part + "_labels"], dtype=np.uint8) == self.chosen_label
                ).nonzero()[0]
    ...
    def __getitem__(self, i):
        ...
        if self.shapes_file is None:
            self.images_file = h5.File(
                os.path.join(self.path2data, self.images_fname),
                "r",
                libver="latest",
                swmr=True,
            )
            self.shapes_file = h5.File(
                os.path.join(self.path2data, self.meshes_fname),
                "r",
                libver="latest",
                swmr=True,
        ...
        vertices_c = np.array(
            self.shapes_file[self.part + "_vertices_c"][
                self.vertices_c_bounds[sh_i] : self.vertices_c_bounds[sh_i + 1]
            ],
            dtype=np.float32,
        )
        faces_vc = np.array(
            self.shapes_file[self.part + "_faces_vc"][
                self.faces_bounds[sh_i] : self.faces_bounds[sh_i + 1]
            ],
            dtype=np.uint32,
        )
        sample = sample_cloud(
        ...
        sample.update({"image": self.images_file[self.part + "_images"][im_i]})
