...
def sample_cloud(vertices_c, faces_vc, size=2**10, return_eval_cloud=False):
    ...
    p_sample = np.random.choice(
        np.arange(polygons.shape[0]),
        size=2 * size if return_eval_cloud else size,
        p=probs,
    )

    sampled_polygons = polygons[p_sample]

    s1 = np.random.random((2 * size if return_eval_cloud else size, 1)).astype(
        np.float32
    )
    s2 = np.random.random((2 * size if return_eval_cloud else size, 1)).astype(
        np.float32
    )
    cond = (s1 + s2) > 1.0
    s1[cond] = 1.0 - s1[cond]
    ...
    sample = {
        "cloud": (
            ...
            + s2 * (sampled_polygons[:, 2] - sampled_polygons[:, 0])
        ).astype(np.float32)
    }
