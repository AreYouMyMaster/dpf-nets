...
class Local_Cond_RNVP_MC_Global_RNVP_VAE(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.pc_encoder = PointNetCloudEncoder(
            self.pc_enc_init_n_channels,
        ...
        self.g_prior = GlobalRNVPDecoder(
            self.g_prior_n_flows,
            self.g_prior_n_features,
            self.g_latent_space_size,
            weight_std=0.01,
        )

        self.g_posterior = FeatureEncoder(
            self.g_posterior_n_layers,
            self.pc_enc_n_features[-1],
            self.g_latent_space_size,
            deterministic=False,
            mu_weight_std=0.0033,
            mu_bias=0.0,
            logvar_weight_std=0.033,
            logvar_bias=0.0,
        )

        if self.p_decoder_base_type == "free":
            self.p_prior = FeatureEncoder(
                self.p_prior_n_layers,
                self.g_latent_space_size,
                self.p_latent_space_size,
                deterministic=False,
                mu_weight_std=0.001,
                mu_bias=0.0,
            ...
            self.register_buffer(
                "p_prior_mus", torch.zeros((1, self.p_latent_space_size, 1))
            )
            self.p_prior = FeatureEncoder(
                self.p_prior_n_layers,
                self.g_latent_space_size,
            ...
            self.register_buffer(
                "p_prior_logvar",
                self.p_decoder_base_var * torch.ones((1, self.p_latent_space_size, 1)),
            )

        self.pc_decoder = LocalCondRNVPDecoder(
            self.p_decoder_n_flows,
    ...
    def decode(self, g_sample, n_sampled_points=2048):
        ...
        if self.p_decoder_base_type == "free":
            ...
            output["p_prior_mus"] = [
                output["p_prior_mus"]
                .unsqueeze(2)
                .expand(g_sample.shape[0], self.p_latent_space_size, n_sampled_points)
            ]
    ...
    def forward(self, g_input, p_input, n_sampled_points=None):
        ...
        if self.mode == "training":
            ...
            output["g_posterior_samples"] = self.reparameterize(
                output["g_posterior_mus"], output["g_posterior_logvars"]
            )
            ...
            output["g_prior_samples"] = [
                self.reparameterize(
                    output["g_prior_mus"][0], output["g_prior_logvars"][0]
                )
            ]
            ...
            if self.p_decoder_base_type == "free":
                ...
                output["p_prior_logvars"] = [
                    self.p_prior(output["g_prior_samples"][-1])
                    .unsqueeze(2)
                    .expand(
                        p_input.shape[0], self.p_latent_space_size, sampled_cloud_size
                    )
...
class Local_Cond_RNVP_MC_Global_RNVP_VAE_IC(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.g0_prior = FeatureEncoder(
            self.g_prior_n_layers,
            self.g_latent_space_size,
            self.g_latent_space_size,
            deterministic=False,
            mu_weight_std=0.0033,
    ...
    def forward(self, g_input, p_input, images, n_sampled_points=None):
        ...
        if self.mode == "training":
            ...
            output["g_prior_mus"], output["g_prior_logvars"] = self.g0_prior(
                img_features
            )
            output["g_prior_mus"], output["g_prior_logvars"] = [
                output["g_prior_mus"]
            ], [output["g_prior_logvars"]]
            buf_g = self.g_prior(output["g_posterior_samples"], mode="inverse")
            output["g_prior_samples"] = buf_g[0] + [output["g_posterior_samples"]]
