        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if group["weight_decay"] != 0:
                    p.data.add_(
                        -torch.addcdiv(
                            torch.mul(p.data, group["weight_decay"]),
                            exp_avg_c,
                            denom_c,
                            value=group["lr"],
                        )
...
class LRUpdater(object):
    ...
    def __call__(self, optimizer, epoch, iteration):
        ...
        cur_step = (rel_epoch * self.epoch_length + iteration) / (
            self.cycle_length * self.epoch_length
        )
        cur_lr = self.min_lr + 0.5 * (self.max_lr - self.min_lr) * (
            1.0 + np.cos(np.pi * cur_step)
        )
        cur_beta2 = self.min_beta2 + 0.5 * (self.max_beta2 - self.min_beta2) * (
            1.0 + np.cos(np.pi * cur_step)
        )
