...
def evaluate(iterator, model, loss_func, **kwargs):
    ...
    if is_saving:
        clouds_fname = "{}_{}_{}_{}_clouds_{}.h5".format(
            kwargs["model_name"][:-4],
        ...
        sampled_clouds = clouds_file.create_dataset(
            "sampled_clouds",
            shape=(
                kwargs["N_sets"] * len(iterator.dataset),
                3,
                kwargs["sampled_cloud_size"],
            ),
            dtype=np.float32,
    ...
    for i, batch in enumerate(iterator):
        ...
        if train_mode == "p_rnvp_mc_g_rnvp_vae":
            inf_end = time()
            outputs = model(
                g_clouds, p_clouds, n_sampled_points=kwargs["sampled_cloud_size"]
            )
            ...
            outputs = model(
                ...
                p_clouds,
                images,
                n_sampled_points=kwargs["sampled_cloud_size"],
            )
        ...
        if util_mode == "training":
            ...
            if kwargs["orig_scale_evaluation"]:
                ...
                if kwargs["cloud_translate"]:
                    shift = torch.from_numpy(
                        np.array(kwargs["cloud_translate_shift"]).reshape(1, -1, 1)
                    ).cuda()
                    r_clouds += shift
                    p_clouds += shift
            ...
            if is_saving:
                sampled_clouds[
                    kwargs["batch_size"] * i : kwargs["batch_size"] * i
                    + r_clouds.shape[0]
            ...
            dl, dr = distChamferCUDA(
                torch.transpose(r_clouds, 1, 2).contiguous(),
        ...
        line = "[{cc}/{il}]  \tTime {batch_time.val:.3f} ({batch_time.avg:.3f})".format(
            cc=i + 1, il=len(iterator), batch_time=batch_time
        )
        if util_mode == "training":
    ...
    if util_mode == "evaluating":
        ...
        gen_clouds_buf = torch.transpose(
            torch.cat(gen_clouds_buf, dim=0), 2, 1
        ).contiguous()
        ref_clouds_buf = torch.transpose(
            torch.cat(ref_clouds_buf, dim=0), 2, 1
        ).contiguous()

        gen_clouds_buf = gen_clouds_buf.cpu().numpy()
        gen_clouds_inds = set(np.arange(gen_clouds_buf.shape[0]))
        nan_gen_clouds_inds = set(
            np.isnan(gen_clouds_buf).sum(axis=(1, 2)).nonzero()[0]
        )
        gen_clouds_inds = list(gen_clouds_inds - nan_gen_clouds_inds)
        dup_gen_clouds_inds = np.random.choice(
            gen_clouds_inds, size=len(nan_gen_clouds_inds)
        )
        ...
        jsd = JSD(
            gen_clouds_buf.cpu().numpy(),
            ref_clouds_buf.cpu().numpy(),
            clouds1_flag="gen",
            clouds2_flag="ref",
            warning=False,
        )
    ...
    if is_saving and kwargs["N_sets"] > 1:
        for i in range(1, kwargs["N_sets"], 1):
            for j, batch in enumerate(iterator):
                ...
                sampled_clouds[
                    i * len(iterator.dataset)
                    + kwargs["batch_size"] * j : i * len(iterator.dataset)
                    + kwargs["batch_size"] * j
                    + r_clouds.shape[0]
                ] = (r_clouds.cpu().numpy().astype(np.float32))
                gt_clouds[
