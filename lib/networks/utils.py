...
def f_score(predicted_clouds, true_clouds, threshold=0.001):
    ...
    precision = 100.0 * (rd < threshold).float().mean(1)
    recall = 100.0 * (ld < threshold).float().mean(1)
    return 2.0 * precision * recall / (precision + recall + 1e-7)


def get_voxel_occ_dist(
    all_clouds, clouds_flag="gen", res=28, bound=0.5, bs=128, warning=True
):
    if np.any(np.fabs(all_clouds) > bound) and warning:
        print(
            "{} clouds out of cube bounds: [-{}; {}]".format(clouds_flag, bound, bound)
        )
    ...
    for i in range(nbs):
        ...
        preiis = np.logical_and(
            v_bs[:28].reshape(-1, 1) <= preiis, preiis < v_bs[1:].reshape(-1, 1)
        )
        iis = preiis.argmax(0)
        iis_values = preiis.sum(0) > 0

        prejjs = clouds[:, :, 1].reshape(1, -1)
        prejjs = np.logical_and(
            v_bs[:28].reshape(-1, 1) <= prejjs, prejjs < v_bs[1:].reshape(-1, 1)
        )
        jjs = prejjs.argmax(0)
        jjs_values = prejjs.sum(0) > 0

        prekks = clouds[:, :, 2].reshape(1, -1)
        prekks = np.logical_and(
            v_bs[:28].reshape(-1, 1) <= prekks, prekks < v_bs[1:].reshape(-1, 1)
        )
        kks = prekks.argmax(0)
        kks_values = prekks.sum(0) > 0

        values = np.uint64(
            np.logical_and(np.logical_and(iis_values, jjs_values), kks_values)
        )
...
def JSD(clouds1, clouds2, clouds1_flag="gen", clouds2_flag="ref", warning=True):
    ...
    return entropy((dist1 + dist2).flatten() / 2.0, base=2) - 0.5 * (
        entropy(dist1.flatten(), base=2) + entropy(dist2.flatten(), base=2)
    )
