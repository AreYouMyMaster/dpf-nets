    ...
    def __init__(self, init_n_channels, init_n_features, n_features):
        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    (
                        "init_sd",
                        SharedDot(init_n_channels, init_n_features, 1, bias=False),
                    ),
        ...
        for i in range(len(self.n_features)):
            ...
            self.features.add_module(
                "sd{}".format(i), SharedDot(cur_features, n_features[i], 1, bias=False)
            )
    ...
    def __init__(
        ...
        if n_layers > 0:
            ...
            for i in range(n_layers):
                self.features.add_module(
                    "mlp{}".format(i), nn.Linear(in_features, in_features, bias=False)
                )
                if self.batch_norm:
                    self.features.add_module(
                        "mlp{}_bn".format(i), nn.BatchNorm1d(in_features)
                    )
            ...
            self.logvars = nn.Sequential(
                OrderedDict(
                    [
                        (
                            "logvar_mlp0",
                            nn.Linear(in_features, latent_space_size, bias=True),
                        )
                    ]
                )
            )
