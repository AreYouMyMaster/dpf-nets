...
class CondRealNVPFlow3D(nn.Module):
    def __init__(
        ...
        self.T_mu_0_cond_w = nn.Sequential(
            OrderedDict(
                [
                    (
                        "mu_sd1_film_w0",
                        nn.Linear(self.g_n_features, self.f_n_features, bias=False),
                    ),
                    ("mu_sd1_film_w0_bn", nn.BatchNorm1d(self.f_n_features)),
                    ("mu_sd1_film_w0_swish", Swish()),
                    (
                        "mu_sd1_film_w1",
                        nn.Linear(self.f_n_features, self.f_n_features, bias=True),
                    ),
                ]
            )
        )

        self.T_mu_0_cond_b = nn.Sequential(
            OrderedDict(
                [
                    (
                        "mu_sd1_film_b0",
        ...
        self.T_mu_1 = nn.Sequential(
            OrderedDict(
                [
                    ("mu_sd1_relu", nn.ReLU(inplace=True)),
                    (
                        "mu_sd2",
                        SharedDot(self.f_n_features, len(self.warp_inds), 1, bias=True),
                    ),
                ]
            )
        )
        ...
        self.T_logvar_0 = nn.Sequential(
            OrderedDict(
                [
                    (
                        "logvar_sd0",
                        SharedDot(len(self.keep_inds), self.f_n_features, 1),
                    ),
        ...
        self.T_logvar_0_cond_w = nn.Sequential(
            OrderedDict(
                [
                    (
                        "logvar_sd1_film_w0",
        ...
        self.T_logvar_0_cond_b = nn.Sequential(
            OrderedDict(
                [
                    (
                        "logvar_sd1_film_b0",
    ...
    def forward(self, p, g, mode="direct"):
        ...
        logvar[:, self.warp_inds, :] = nn.functional.softsign(
            self.T_logvar_1(
                torch.add(self.eps, torch.exp(self.T_logvar_0_cond_w(g).unsqueeze(2)))
                * self.T_logvar_0(p[:, self.keep_inds, :].contiguous())
                + self.T_logvar_0_cond_b(g).unsqueeze(2)
            )
        )

        mu[:, self.warp_inds, :] = self.T_mu_1(
            torch.add(self.eps, torch.exp(self.T_mu_0_cond_w(g).unsqueeze(2)))
            * self.T_mu_0(p[:, self.keep_inds, :].contiguous())
            + self.T_mu_0_cond_b(g).unsqueeze(2)
        )
...
class RealNVPFlow(nn.Module):
    def __init__(
        self, n_features, g_n_features, weight_std=0.01, warp_inds=[0], eps=1e-6
    ):
        ...
        self.T_mu_0 = nn.Sequential(
            OrderedDict(
                [
                    (
                        "mu_mlp0",
                        nn.Linear(len(self.keep_inds), self.n_features, bias=False),
                    ),
                    ("mu_mlp0_bn", nn.BatchNorm1d(self.n_features)),
                    ("mu_mlp0_swish", Swish()),
                    (
                        "mu_mlp1",
                        nn.Linear(self.n_features, len(self.warp_inds), bias=True),
                    ),
                ]
            )
        )
    ...
    def forward(self, g, mode="direct"):
        ...
        logvar[:, self.warp_inds] = torch.log(
            torch.add(
                self.eps, torch.exp(self.T_logvar_0(g[:, self.keep_inds].contiguous()))
            )
        )
...
class RealNVPFlowCouple(nn.Module):
    def __init__(self, n_features, g_n_features, weight_std=0.01, pattern=0):
        ...
        if pattern == 0:
            self.nvp1 = RealNVPFlow(
                n_features,
                g_n_features,
                weight_std=weight_std,
                warp_inds=list(np.arange(g_n_features)[::2]),
            )
            self.nvp2 = RealNVPFlow(
                n_features,
                g_n_features,
                weight_std=weight_std,
                warp_inds=list(np.arange(g_n_features)[1::2]),
            )
        elif pattern == 1:
            self.nvp1 = RealNVPFlow(
                n_features,
                g_n_features,
                weight_std=weight_std,
                warp_inds=list(np.arange(g_n_features)[: g_n_features // 2]),
            )
            self.nvp2 = RealNVPFlow(
                n_features,
                g_n_features,
