...
class ObjMesh(object):
    def __init__(self, filename):
        ...
        for line_ind in np.array(
            list(map(lambda l: l == "" or l[0] == "#", raw_obj_data))
        ).nonzero()[0][::-1]:
        ...
        for line in raw_obj_data:
            ...
            if line_segs[0] == "v":
                self.vertices = np.append(
                    self.vertices,
                    np.array(list(map(lambda x: np.float32(x), line_segs[1:]))).reshape(
                        1, -1
                    ),
                    axis=0,
                )
    ...
    def cleanup(self):
        ...
        for i in range(len(self.vertices) - 1):
            tmp = np.sqrt(
                ((self.vertices[(i + 1) :] - self.vertices[i].reshape(1, -1)) ** 2).sum(
                    1
                )
            )
            tmp = np.isclose(tmp, 0.0, atol=5e-6).nonzero()[0] + i + 1
        ...
        for key, group in self.groups.items():
            ...
            mask = np.logical_not(np.isclose(group_ps_areas, 0.0, atol=1e-10))
            self.groups[key]["faces_v"] = group["faces_v"][mask]
        ...
        for key, group in self.groups.items():
            ...
            mask = np.logical_not(
                np.isclose(np.fabs((tmp2 * tmp1).sum(1)), 1.0, rtol=1e-5)
            )
            self.groups[key]["faces_v"] = group["faces_v"][mask]

        # Remove duplicate faces in each group #
        for key, group in self.groups.items():
            unique_faces_inds = list(
                OrderedDict(
                    list(
                        map(
                            lambda i, x: (frozenset(x), i),
                            range(len(group["faces_v"])),
                            group["faces_v"],
                        )
                    )
                ).values()
        ...
        unique_faces_inds = set(
            OrderedDict(
                list(
                    map(
                        lambda i, x: (frozenset(x), i),
                        range(len(all_faces_v)),
                        all_faces_v,
                    )
                )
            ).values()
        ...
        areas = (
            np.sqrt(
                (
                    np.cross(
                        polygons[:, 2] - polygons[:, 0], polygons[:, 2] - polygons[:, 1]
                    )
                    ** 2
                ).sum(1)
            )
